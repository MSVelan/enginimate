...
class WorkerSteps(BaseModel):
    steps: list[WorkerStep] = Field(
        default_factory=list, description="Steps required for scene creation"
    )


def planner(state: State):
    planner_llm = llm.with_structured_output(WorkerSteps)
    pipeline = prompt_template | planner_llm
    worker_steps = pipeline.invoke({"scene_description": state.prompt})
    return {"steps": worker_steps.steps}
